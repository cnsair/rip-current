...
class _ResBlock(nn.Module):
    ...
    CRITICAL: attribute names here MUST be identical to _TimeCondResBlock
    in train_diffusion_seg.py.  PyTorch serialises weights by attribute
    path (e.g. enc1.norm1.weight).  Any rename — even shortening norm1 to
    n1 — produces a key mismatch and a RuntimeError at load_state_dict.
    The original evaluate_test_set.py used shortened names (n1, c1, tp, sk)
    which is why evaluation crashed with hundreds of missing/unexpected keys.
    """
    def __init__(self, in_ch, out_ch, td):
        ...
        self.time_proj = nn.Linear(td, out_ch * 2)
        self.skip      = nn.Conv2d(in_ch, out_ch, 1) \
                         if in_ch != out_ch else nn.Identity()
    def forward(self, x, te):
        h = F.silu(self.norm1(x))
        h = self.conv1(h)
        sc, sh = self.time_proj(te).chunk(2, dim=-1)
